...
const getCachedTrio = unstable_cache(
    async (season: string) => {
        const firestore = getAdminDb();
        const trioQ = firestore.collectionGroup('opinions').where('type', '==', 'trio');
        const querySnapshot = await trioQ.get();
        ...
        if (matchIds.length > 0) {
            await Promise.all(matchIds.map(async (mid) => {
                try {
                    ...
                    console.error(`Error processing trio match ${mid}:`, e);
                }
            }));
        }
...
export async function GET(request: Request) {
    try {
        ...
        const data = await getCachedTrio(season);
        return NextResponse.json(data);
    } catch (error: any) {
        console.error('Public Trio API Error:', error);
        return NextResponse.json({
            error: 'Failed to fetch trio data',
