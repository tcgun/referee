...
export default function TrioPage() {
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                setLoading(true);
                const res = await fetch(`/api/public/trio?season=${selectedSeason}`);
                if (!res.ok) throw new Error('Failed to fetch Trio data');
